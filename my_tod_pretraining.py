...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The output directory where the model predictions and checkpoints will be written. If not provided it will be automatically named in exp/",
    )
    parser.add_argument(
        "--model_type",
        default="bert",
    ...
    parser.add_argument(
        "--example_type",
        help='type in ["turn", "dial"] for data reading',
        required=False,
        default="turn",
    )
    ...
    # Initialize wandb early so we can monitor also the system usage
    # Do not want wandb? Run wandb disabled in current dir or set WANDB_DISABLED=1
    if args.local_rank in [-1, 0]:
        wandb_run = wandb.init(
            entity="keya-dialog",
            project="ToD-BERT-baseline-pretrain",
            dir=args.output_dir,
            config=vars(args),
        )
    ...
    if args.do_train:
        ...
        for ds_name in ast.literal_eval(args.dataset):
            ...
            if ds_name in ast.literal_eval(args.holdout_dataset):
                ...
                datasets[ds_name] = {
                    "train": data_trn + data_dev + data_tst,
                    "dev": [],
                    "test": [],
                    "meta": data_meta,
                }
        ...
        global_step, tr_loss = train(
            args, trn_loader, dev_loader, model, tokenizer, cand_uttr_sys_dict, others, wandb_run
        )
...
def train(args, trn_loader, dev_loader, model, tokenizer, cand_uttr_sys_dict, others, wandb_run):
    """Train the model"""
    if args.max_steps > 0:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.add_rs_loss:
                loss, loss_mlm, loss_rs = compute_loss(batch)
            else:
                loss, loss_mlm = compute_loss(batch)
    ...
    if args.local_rank in [-1, 0]:
        # OLD:tb_writer.close()
        # wandb_run.close() actually we do not want to close, we want to log also evaluation
        ...
...
class mlm_and_rs_loss:
    def __init__(self, model, tokenizer, cand_uttr_sys_dict, args, others):
        ...
        self.cand_uttr_sys_dict = cand_uttr_sys_dict
        self.args = args
    ...
    def __call__(self, batch):
        ...
        input_cont = input_cont.to(self.args.device)
        input_resp = input_resp.to(self.args.device)
        resp_label = resp_label.to(self.args.device)
        ...
        loss_rs = self.xeloss(scores, resp_label)
        loss += loss_rs
        loss_rs = loss_rs.item()
        return loss, loss_mlm, loss_rs
